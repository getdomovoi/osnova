...
describe("format 9", () => {
  it("writes core, sha, edges and text with string tables", async () => {
    ...
    expect(core.formatVersion).toBe(10);
  ...
  it("returns undefined for a pre-bump format 9 extraction identity and rebuilds bytes equal to a full build", async () => {
    ...
    expect(original).toContain('"structural-9.23.scan-4');
    const stale = Buffer.from(original.replace('"structural-9.23.scan-4', '"structural-9.scan-4'), "utf8");
