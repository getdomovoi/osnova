...
- Edge semantics v1: direct calls, imports, name references and declared heritage (`extends` edges, from a written superclass or interface name) only. No type inference. Resolution preference: same file, then files reached by the file's resolved imports, then unique name within the language family. Ambiguous candidates at the preferred tier remain unresolved; persist the resolution basis and never label a name heuristic as type-proven evidence.
