...
- The MCP tool names and argument shapes (`osnova_ground`, `osnova_thread`, `osnova_outline`, `osnova_warp`, `osnova_groundwork`, `osnova_footing`, `osnova_settle`, plus the deprecated aliases `osnova_ask`, `osnova_find_text`, `osnova_skeleton`, `osnova_callers`, `osnova_map` until the next release) and the exported API (`buildIndex`, `loadIndex`, `applyChanges`, `freshness`, `ask`, `findText`, `skeleton`, `callers`, `map`, `renderMapCard`, `runMcpStdio`, `cacheDir` parameter, `OSNOVA_CACHE_DIR`) are a frozen contract for a downstream consumer. Breaking them requires a coordinated major version, never a surprise.
