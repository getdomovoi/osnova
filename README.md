...
Artifact format 7 persists diagnostics, export names, re-export links, lexical/receiver hints, member kinds and resolution evidence. `loadIndex` returns `undefined` for format-1 through format-6 caches, which predate current analysis guarantees; query commands rebuild them. Corrupt or unsupported newer artifacts and failed cache writes remain explicit errors. Repaired files clear their old diagnostics on incremental update.
