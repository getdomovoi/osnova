...
Artifact format 8 splits the cache into a structural core (`index.json`) and a text sidecar (`text.bin`). The core carries diagnostics, export names, re-export links, lexical/receiver hints, member kinds, resolution evidence and byte offsets into the sidecar; source text is read lazily per file on first use and verified against its recorded hash. `loadIndex` returns `undefined` for format-1 through format-7 caches; query commands rebuild them. A core whose sidecar is missing or mismatched is reported as `cache-read-failed`, never partially trusted. Corrupt or unsupported newer artifacts and failed cache writes remain explicit errors. Repaired files clear their old diagnostics on incremental update.
