...
Artifact format 5 persists diagnostics, export names, lexical bindings and resolution evidence, and invalidates the older nested-definition coverage. `loadIndex` returns `undefined` for format-1 through format-4 caches, which predate current analysis guarantees; query commands rebuild them. Corrupt or unsupported newer artifacts and failed cache writes remain explicit errors. Repaired files clear their old diagnostics on incremental update.
