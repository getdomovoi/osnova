...
`resolutionCoverage(index)` returns per-language and total rows of files, symbols, call edges, resolved, ambiguous and unresolved counts, resolved counts by method, unresolved counts by reason, the share of resolved call sites, the count of call sites whose import target lies outside the index (`externalCalls`, the `import-target-unresolved` reason) and the share of resolved call sites among the rest. Limitations are exactly `indexed-call-sites-only`, `resolution-is-heuristic-not-type-inference`, `unindexed-files-not-counted`, `external-calls-are-import-target-unresolved-edges`. `scripts/coverage-corpora.mjs` runs it on pinned checkouts and refuses a checkout whose revision differs from its manifest or that has modified tracked files.

`plumb(index, symbol, claims, { direction, depth })` takes claims as `{ file, line }` (parse `path:line` strings with `parseClaims`, which de-duplicates and rejects malformed input) and returns one verdict per claim: `confirmed` when a resolved edge from `callersDetailed` at the requested depth sits at that file and line; `name-only` when an unresolved or ambiguous call edge at that line names the target; `no-call` when the file is indexed and no call at that line names the target; `not-indexed` when the file is not in the index. `missing` lists resolved edges at the requested depth that no claim covers, sorted by file then line. Limitations are exactly `confirmed-means-indexed-resolved-edge-not-runtime-proof`, `name-only-is-a-heuristic-match`, `missing-covers-indexed-resolved-edges-only`. A bare name that matches several symbols returns `status: "ambiguous"` with the candidate list and no verdicts. CLI `plumb` and MCP `osnova_plumb` print the same text under 4096 code units.
