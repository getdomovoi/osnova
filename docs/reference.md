...
For TypeScript/JavaScript and Python, member calls use a syntax-backed receiver hint rather than a global method-name match. Supported evidence is an explicit class reference, a direct constructor site or local constructor assignment, lexical JavaScript `this` in indexed methods (including nested arrow capture), an unshadowed Python method receiver, or a Python parameter whose annotation is a plain or dotted class name that resolves to an indexed class (basis `annotation`; unions, `Optional[...]`, string annotations and reassigned parameters stay unbound). The declared class and member kind must agree; missing/inherited members, conflicting member identities, properties returning callables and unknown receivers stay unresolved. `receiver-hint` evidence records the class symbol, class/instance mode and basis, retaining export hops when the class came through a barrel.
...
`plumb(index, symbol, claims, { direction, depth })` takes claims as `{ file, line }` (parse `path:line` strings with `parseClaims`, which de-duplicates and rejects malformed input) and returns one verdict per claim: `confirmed` when a resolved edge from `callersDetailed` at the requested depth sits at that file and line; `name-only` when an unresolved or ambiguous call edge at that line names the target; `no-call` when the file is indexed and no call at that line names the target; `not-indexed` when the file is not in the index. `missing` lists resolved edges at the requested depth that no claim covers, sorted by file then line. Only call edges count; references and imports never confirm a claim. Limitations are exactly `confirmed-means-indexed-resolved-edge-not-runtime-proof`, `name-only-is-a-heuristic-match`, `missing-covers-indexed-resolved-edges-only`, `call-edges-only`. A bare name that matches several symbols throws an error naming the candidates, so a claim list is never silently discarded. Claim paths must be repository-relative: no absolute paths, drive letters, backslashes or parent segments. CLI `plumb` and MCP `osnova_plumb` print the same text under 4096 code units.
