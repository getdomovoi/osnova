// Shell parsing for the session ledger. It lives beside the ledger so the ledger reads recorded commands without
// depending on hook code; it runs nothing and only tokenizes.
...
export function shellWords(command: string): readonly string[] | null {
  const tokens = tokenize(command);
  if (tokens === null || tokens.some((token) => token.kind !== "word" || token.word.dynamic)) return null;
  return tokens.map((token) => (token as { kind: "word"; word: Word }).word.text);
}
...
function pipelines(tokens: readonly Token[]): Pipeline[] {
  ...
  for (const token of tokens) {
    ...
    if (token.op === "redirect" || token.op === "input") { skipNext = true; continue; }
