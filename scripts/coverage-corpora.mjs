...
const { buildIndex, resolutionCoverage } = await import(path.join(root, "dist", "index.js"));
...
for (const [name, dir] of [...workspaces].sort(([a], [b]) => (a < b ? -1 : 1))) {
  const manifest = JSON.parse(readFileSync(path.join(root, "benchmarks", `${name}-v1.json`), "utf8"));
  ...
  try {
    ...
    corpora.push({ corpus: manifest.id, repository: manifest.source.repository, revision: head, generation: report.generation, total: report.total, languages: report.languages, limitations: report.limitations });
    console.log(`${name}: ${report.total.resolved}/${report.total.calls} call sites resolved (${(report.total.resolvedShare * 100).toFixed(1)}%)`);
