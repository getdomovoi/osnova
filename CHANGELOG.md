...
- `osnova_warp` and `warp` print the call-site line under each confirmed relationship (`  > return target(7);`, trimmed, at most 160 code units) so a site can be checked without opening the file; the three-harness trial showed agents re-reading five to seven files per answer to confirm lines. Under the budget the text is dropped before any relationship is. To make room inside the unchanged 2,048 code unit budget, a relationship whose call site is the relationship's own `file:line` (every `in` result) no longer repeats it as `source file:line`; `out` results keep the suffix because the site and the target differ.
